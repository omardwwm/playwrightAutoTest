...
const TOTAL_SHARDS = process.env.CI ? 5 : 1;
  ...
  return new Promise((resolve, reject) => {
    ...
    exec(command, (error, stdout, stderr) => {
      if (error) {
        ...
        console.log(
          `Allure report generated at ${allureReportDir}:\n${stdout}`
        );
        resolve(stdout);
      }
    });
...
const runTest = async (locale, realm, environment, shardIndex) => {
  console.log(
    `RUNNING TEST FOR LOCALE=> ${locale} in REALM=> ${realm} in ENVIRONMENT=> ${environment}` +
...
console.log(
  `Parsed arguments - Locale: ${givenLocale}, Realm: ${givenRealm}, Environment: ${givenEnvironment}, Shard: ${givenShard}`
);
...
(async () => {
  try {
    ...
    try {
      ...
      const allureReportDir = `docs/${givenEnvironment}/allure-report-${givenEnvironment}`;
      await generateAllureReport(allureResultsDir, allureReportDir);
      console.log(
        `Allure report generated successfully at ${allureReportDir}.`
      );
    } catch (error) {
